...
class ListsSuite extends FunSuite {
  ...
  test("sum of a few numbers with zeros and negative") {
    assert(sum(List(-1,-2,0,1,2,2)) === 2)
  }
  ...
  test("max of a few numbers with zeros and negative") {
    assert(max(List(-1,-2,0,1,2,2)) === 2)
  }
}
